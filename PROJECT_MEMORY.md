...
_Last updated: 2026-04-27 — Messaging overhaul session. (1) Run chat cleanup: `RUN_CHAT_EXPIRY_MS` bumped 4h→24h; swipe-to-dismiss added to run chat inbox rows (`dismissRunChat` in runChatService.js, `isDismissed` filter in useMyRunChats.js, dismiss action + red Remove button in MessagesScreen.js); `return true` fallback bug fixed → `return false` so old runs with no time info are hidden. (2) Run chat push notifications: new `onRunChatMessageCreated` Cloud Function (triggers on `runs/{runId}/messages/{messageId}`, fans out to all participants except sender, respects `isMuted`, 30s cooldown per recipient). (3) Deep-link tap handler for run chat notifications added to App.js. (4) DM read receipts: `subscribeToConversation` added to dmService.js; `otherLastSeenMs` state + `lastOwnMessageId` memo in DMConversationScreen; "Read" label shown under last own message once other user has opened chat. (5) In-app notification permission: SettingsScreen.js now checks OS permission on mount + on focus via `Notifications.getPermissionsAsync()`; shows orange blocked banner with "Open Settings" button when permission denied; toggle intercepted with settings nudge if OS blocked; run chat notification type added to the list. (6) DM media messages: images (camera + library) and GIFs (Giphy picker) implemented in DMConversationScreen; `sendDMMedia` added to dmService.js; Storage rules updated with `dmImages/` path (10MB max); full-screen media viewer modal; `+` attachment button in input bar; `onDmMessageCreated` Cloud Function updated for image/gif notification text. (7) SESSION_START.md: Deploy Commands section added permanently at top. (8) Parked: audio messages, reply-to-message, stickers → PARKING_LOT.md. 2026-04-25 — Extended presence / long session support implemented. `services/presenceService.js`: added `extendPresence(odId, gymId)` — transactional write that bumps `expiresAt` to `now + 2h` on both the presence doc and `users.activePresence.expiresAt`. `hooks/usePresence.js`: added GPS monitoring effect (polls every 60s while checked in); extends session when `expiresAt - now < 30 min` and GPS confirms user is still inside gym radius; auto-checks-out after 2 consecutive out-of-radius readings. No Cloud Function changes — `expirePresence` CF handles abandonment naturally when app is closed > 2h. Added `import * as Location from 'expo-location'`, `isLocationGranted`, `extendPresence as extendPresenceService` to usePresence imports; added `useRef` to React import. 2026-04-01 — Production iOS build shipped, app live on TestFlight. 2026-04-01: app.json plugins array removed (config-plugins version mismatch); permission strings added to ios.infoPlist directly; buildNumber set to "2"; autoIncrement:true added to eas.json production profile; .easignore expanded to trim upload; project artifacts cleaned (dist/, .expo 2/, ios/RunCheck 2.xcodeproj and all ios/* 2 duplicates removed); EAS build eaee52e1 succeeded at 12.3MB; app now live on device via TestFlight. 2026-03-30 — weekly cleanup + Gym Batches 3–6 + selectBestGymPhoto.js (2026-03-30: Gym Batches 3–6 added to seedProductionGyms.js; total now 89 gyms seeded to Firestore; selectBestGymPhoto.js new AI photo selection script added to runcheck-backend/scripts/. 2026-03-29: Gym Batch 2 — 10 new gyms added to seedProductionGyms.js; enrichGymsWithPlaces.js SOURCE_GYMS updated with Batch 2; dotenv path fixed in enrichGymsWithPlaces.js — uses scripts/.env with GOOGLE_MAPS_API_KEY fallback; Batch 2 gyms pending enrichment pipeline for placeId/website/photos. 2026-03-27: Firestore composite index fix for AdminAllClipsScreen; expireClips.ts raw-file safety guard; age validation + COPPA hardening on SignupScreen; OnboardingHomeCourtScreen overhaul with location + search + request-gym row; RequestGym navigation fixed for onboarding scope; ProfileStack back-button fixed; storagePath vs finalStoragePath root cause discovered and reverted across 7 files. 2026-03-26 session C: skeleton/freeze issue fully resolved — true root cause was App.js Runs tabPress listener using navigate() causing competing stack animations in new arch; fixed with StackActions.popToTop() targeted at RunsStack state key; all six subscription hooks also cleaned of InteractionManager/setTimeout delays and debug logs removed. 2026-03-26 session B: backend fully deployed and verified — all Cloud Functions including Phase 2 push notifications live; Firestore rules deployed and verified matching local; serviceAccountKey.json confirmed not in git history, no rotation required. 2026-03-26 session A: RC-002 complete fix — HomeScreen.js reactive run-doc subscriptions (onSnapshot per feed run) remove stale cards for all last-leaver scenarios; firestore.rules activity delete rule extended for 'started a run at' docs; leaveRun fire-and-forget delete now works for non-creator last-leavers; hooks refactored with InteractionManager deferred subscriptions (frozen-skeleton fix), utils/sanitize.js added as centralized input sanitizer, HomeScreen updated. 2026-03-25 session: added notifyFollowersRunCreated + notifyFollowersPresenceMilestone + onGymPresenceUpdated Cloud Functions (Phase 2 push notifications — gym follow alerts + live activity alerts), ViewRunsScreen UI polish pass (blinking status dots, follow banner, status color tiers), reliability formula repair + deploy-pending fix, repairReliabilityScores.js admin script added)_
...
- **Gym Batches 3–6 — 70 new gyms seeded (2026-03-30)**: `seedProductionGyms.js` now has **89 gyms total**. Batch 3 (37 gyms): UT facilities (Gregory Gym, UT Rec Sports Center), 7 YMCA branches (East Communities, Southwest Family, Northwest Family, North Austin, Cedar Park, Twin Lakes, Hays Communities, Four Points), 10 Austin PARD rec centers (Austin Rec, South Austin Rec, Givens, Gus Garcia, Parque Zaragoza, Virginia L. Brown, Dottie Jordan, Turner Roberts, Dove Springs, West Austin, Rosewood, Wells Branch, Del Valle), 2 Austin Sports Centers, Life Time Fitness South Austin, 24 Hour Fitness x2, LA Fitness x2, 6 suburb rec facilities (Georgetown, Pflugerville, NE Metro Park, Brushy Creek Community, Kyle, Lakeway, Bee Cave). Batch 4 (7 gyms): Round Rock Sports Center, CHASCO Family YMCA, LA Fitness AW Grimes, LA Fitness RM 620, 24 Hour Fitness Round Rock, 24 Hour Fitness Pflugerville, Hutto Family YMCA. Batch 5 (9 gyms): Outdoor courts at Austin city parks (Alamo Pocket, Shipe, Northwest District, Hancock, Givens, Buttermilk, South Austin Rec outdoor, Dottie Jordan, Eastwoods). Batch 6 (17 gyms): Outdoor park courts — Round Rock, Pflugerville, Hutto suburbs (Frontier, Lake Creek, Green Slopes, Dell Way, Settlement, Chandler Creek, Cat Hollow, Sendero Springs, Pfluger, Highland, Heatherwilde, Hutto Community, Fritz, Brushy Creek Sports, Milburn, Nelson Ranch, Creekside). All Batches 2–6 gyms pending enrichment pipeline.
- **`selectBestGymPhoto.js` — new AI photo selection script (2026-03-30)**: `runcheck-backend/scripts/selectBestGymPhoto.js`. Uses Claude vision (claude-haiku) to score every candidate photo per gym and select the best basketball-court thumbnail. Priority order: manual override (`customThumbnails.json`) → best AI-scored court photo → first photo fallback → null (flag for manual review). Input: `enrichedGymsWithImages.json` + `customThumbnails.json`. Outputs: `enrichedGymsWithImages.json` (updated in-place with best `imageUrl`), `photoReviewNeeded.json`, `photoScoreLog.json`. Requires `ANTHROPIC_API_KEY` in `scripts/.env`. CLI flags: `--dry-run` (preview only), `--gym <id>` (single gym). Run after `downloadGymPhotos.js` and before `seedGyms.js` in the enrichment pipeline.
- **Gym Batch 2 — 10 new gyms seeded (2026-03-29)**: `seedProductionGyms.js` Batch 2 adds: YMCA of Austin Northwest Branch, YMCA of Austin Downtown, YMCA of Austin Round Rock, Cedar Park Recreation Center, LA Fitness Cedar Park, Dittmar Recreation Center (Austin PARD), Metz Recreation Center (Austin PARD), Northwest Recreation Center (Austin PARD), Old Settlers Park Round Rock (outdoor), LA Fitness South Austin. Coordinates are best-effort estimates — pin-drop verify in Google Maps before production.
- **Gym enrichment pipeline hardened (2026-03-29)**: `runcheck-backend/scripts/enrichGymsWithPlaces.js` — dotenv now loads from `scripts/.env` (root `.env` does not exist); `const API_KEY = process.env.GOOGLE_PLACES_API_KEY || process.env.GOOGLE_MAPS_API_KEY` fallback added to match env file key name. `SOURCE_GYMS` updated with all batches. `downloadGymPhotos.js` and `seedGyms.js` already had correct dotenv paths — unchanged. Full pipeline command: `cd ~/Desktop/runcheck-backend && node scripts/enrichGymsWithPlaces.js && node scripts/downloadGymPhotos.js && node scripts/selectBestGymPhoto.js && node scripts/seedGyms.js`.
...
**2026-03-23 through 2026-03-29** — Week 13 hardening sprint. Run Level Phases 2/3/4 (Competitive Meter with skill snapshot from `runParticipants`, "Balanced" label, tappable info sheets). DM hardening: user blocking, conversation muting, message-level report+block, admin `removeDmMessage` CF, `adminActions` audit log. Reliability threshold fix (displayScore pinned to 100 until 3 events). Phase 2 push notifications written and fully deployed 2026-03-26 (`notifyFollowersRunCreated`, `notifyFollowersPresenceMilestone`, `onGymPresenceUpdated`, `detectRunNoShows`, `onScheduleWrite`). `utils/sanitize.js` centralized input sanitizer. `repairReliabilityScores.js` admin script. Hooks InteractionManager refactor (all 6 subscription hooks). RC-002 complete fix (reactive run-doc subscriptions). Skeleton/freeze root cause found and fixed (App.js tabPress → StackActions.popToTop). Backend fully deployed 2026-03-26 (all Cloud Functions + Firestore rules verified). COPPA age validation + OnboardingHomeCourtScreen overhaul + ProfileStack back-button fix (2026-03-27). AdminAllClipsScreen composite indexes + `expireClips` safety guard deployed (2026-03-28). Gym Batch 2: 10 new gyms seeded (total 19); enrichment pipeline scripts updated; `onRunCreatedNotifyScheduledVisitors` CF written (2026-03-29, **not yet deployed**).
...
_See `docs/session-handoffs/` for full per-file tables from sessions prior to 2026-03-12. All features are captured in the Session History Summary above._
