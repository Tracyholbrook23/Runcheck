...
_Last updated: 2026-03-25 (2026-03-25 session: added notifyFollowersRunCreated + notifyFollowersPresenceMilestone + onGymPresenceUpdated Cloud Functions (Phase 2 push notifications — gym follow alerts + live activity alerts), ViewRunsScreen UI polish pass (blinking status dots, follow banner, status color tiers), reliability formula repair + deploy-pending fix)_
...
**Deploy required:** `firebase deploy --only functions:onGymPresenceUpdated,functions:notifyFollowersPresenceMilestone`
(Also still pending from earlier session: `firebase deploy --only functions:notifyFollowersRunCreated,functions:detectRunNoShows,functions:onScheduleWrite`)
