...
- [x] **Stale activity cleanup (RC-002)** — client-side async cross-reference in HomeScreen.js filters out `'started a run at'` activity when the referenced run doc is missing, removed, or has `participantCount <= 0`. Includes `gen` counter + `cancelled` flag to prevent stale-async and post-unmount setState.
