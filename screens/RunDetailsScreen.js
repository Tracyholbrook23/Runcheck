...
export default function RunDetailsScreen({ route, navigation }) {
  ...
  // URL of the clip currently open in the in-app player modal (null = closed).
  ...
  return (
    <SafeAreaView style={styles.safe}>
              ...
              {dailyHighlight && (
                <View style={styles.highlightCard}>
                  ...
                  <ClipCard
                    ...
                    onPlay={() => setActiveClipUrl(clipVideoUrls[dailyHighlight.id])}
                  />
                </View>
              )}
              {gymClips.map((clip) => (
                <ClipCard
                  ...
                  onPlay={() => setActiveClipUrl(clipVideoUrls[clip.id])}
                />
              ))}
            </>
          )}
      ...
      {/* In-app clip player — only mounted while a URL is active */}
      {activeClipUrl && (
        <ClipPlayerModal
          url={activeClipUrl}
          onClose={() => setActiveClipUrl(null)}
        />
      )}
    </SafeAreaView>
  );
 ...
 */
function ClipCard({ clip, videoUrl, thumbnailUri, onPlay }) {
  return (
    <View style={clipPlayerStyles.card}>
      ...
      <Text style={clipPlayerStyles.cardMeta}>
        {clip.likesCount || 0} likes
      </Text>
    </View>
  );
}
...
/**
 * ClipPlayerModal — Mounts only while a clip URL is active (conditional mount
 * at the call site), so the Video component always receives a valid URI.
 * Pauses and unloads the video when the modal is closed to release resources.
 */
function ClipPlayerModal({ url, onClose }) {
  ...
  const handleClose = async () => {
    try {
      ...
      // Ignore errors during teardown — the modal is closing regardless.
    }
    onClose();
  };
  ...
  return (
    ...
    >
      <View style={clipPlayerStyles.container}>
        <Video
          ref={videoRef}
          style={clipPlayerStyles.video}
          source={{ uri: url }}
          shouldPlay
          useNativeControls
        ...
        />
        <TouchableOpacity
          style={clipPlayerStyles.closeButton}
          onPress={handleClose}
          hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
        >
